...
public class MorphAnalyzer {
    ...
    public List<AnalysisOutput> analyze(String input, int pos) throws MorphException {
		...
		Map<String, AnalysisOutput> stems = new HashMap<String, AnalysisOutput>();
		...
		for (AnalysisOutput o : candidates) {
			...
			if (o.getScore() == AnalysisOutput.SCORE_CORRECT && o.getPos() != PatternConstants.POS_NOUN) {
				...
				if (o.getPos() == PatternConstants.POS_NOUN) {
					...
					noun = o;
				        ...
				        || (o.getPatn() == noun.getPatn() && o.getJosa() != null && noun.getJosa() != null && o
				                .getJosa().length() > noun.getJosa().length())) {
					results.remove(noun);
					addResults(o, results, stems);
					noun = o;
				}
			...
			} else if (o.getPos() == PatternConstants.POS_NOUN && o.getCNounList().size() > 0 && !hasCorrect
			        && !hasCorrectNoun) {
		...
		if (results.size() == 0) {
			AnalysisOutput output = new AnalysisOutput(input, null, null, PatternConstants.PTN_N,
			        AnalysisOutput.SCORE_ANALYSIS);
	...
	private void addResults(AnalysisOutput o, List<AnalysisOutput> results, Map<String, AnalysisOutput> stems) {
	...
	public void analysisWithJosa(String stem, String end, List<AnalysisOutput> candidates) throws MorphException {
		...
		if (!DictionaryUtil.existJosa(end) || (chrs.length == 3 && ConstraintUtil.isTwoJosa(end))
		        || (chrs.length == 2 && (ConstraintUtil.isThreeJosa(end)) || "".equals(end)))
		...
		try {
			NounUtil.analysisMJ(output.clone(), candidates);
	...
	public void analysisWithEomi(String stem, String end, List<AnalysisOutput> candidates) throws MorphException {
		...
		try {
			...
			if (entry != null
				...
				char[] features = SyllableUtil.getFeature(stem.charAt(stem.length() - 1)); // ㄹ불규칙일
				                                                                           // 경우
	...
	private boolean constraint(AnalysisOutput o) throws MorphException {
		...
		if ("화해".equals(cnouns.get(cnouns.size() - 1).getWord())) {
			if (!ConstraintUtil.canHaheCompound(cnouns.get(cnouns.size() - 2).getWord()))
				return false;
		} else if (o.getPatn() == PatternConstants.PTN_NSM) {
			if ("내".equals(o.getVsfx()) && cnouns.get(cnouns.size() - 1).getWord().length() != 1) {
				...
				if (entry != null && entry.getFeature(WordEntry.IDX_NCNOUN) != '1')
					return false;
